...
@Composable
fun DaftarUstadz(viewModel: DaftarUstadzViewModel = getViewModel(), navController: NavHostController) {
    ...
    Column(
        ...
        PopSem32("USTADZ/USTADZAH", Primary, Modifier.padding(start = 48.dp))
        Row(
            modifier = Modifier
                .fillMaxWidth(),
            ...
            OutlinedButton(
                onClick = {
                    navController.navigate(Route.AddUstadz.route)
                },
                modifier = Modifier
                    .padding(top = 16.dp),
        ...
        Spacer(modifier = Modifier.padding(top = 16.dp))
        LazyColumn {
            items(state.ustadzs ?: emptyList(), key = { it.id }) { ustadz ->
                ListItem(
                    name = ustadz.name,
                    onClick1 = { /*TODO*/ },
                    onClick2 = { /*TODO*/ }
